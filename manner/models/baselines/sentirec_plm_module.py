...
from torch.nn import CrossEntropyLoss, L1Loss, Linear
...
class SentiRecPLMModule(LightningModule):
    def __init__(
        self,
        plm_model: str,
        ...
        senti_pred_loss_coef: float,
        senti_div_loss_coef: float,
        ...
        self.sentiment_predictor = Linear(self.hparams.text_embedding_dim, 1)
        self.click_predictor = DotProduct()

        self.criterion = CrossEntropyLoss()
        self.senti_pred_loss = L1Loss()
    ...
    def forward(self, batch: MINDRecBatch) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        senti_scores = self.sentiment_predictor(
            torch.cat((candidate_news_vector, clicked_news_vector), dim=0),
        )

        return scores, senti_scores
    ...
    def model_step(self, batch: MINDRecBatch) -> Tuple[
        ...
        scores, senti_scores = self.forward(batch)
        ...
        sentiment_scores = torch.cat(
            (batch["x_cand"]["sentiment_score"], batch["x_hist"]["sentiment_score"])
        )
        senti_pred_loss = self.senti_pred_loss(senti_scores.flatten(), sentiment_scores)
        loss += self.hparams.senti_pred_loss_coef * senti_pred_loss

        # sentiment diversity regularization loss
        senti_hist, mask_hist = to_dense_batch(
            batch["x_hist"]["sentiment_score"], batch["batch_hist"]
        )
        senti_cand, _ = to_dense_batch(batch["x_cand"]["sentiment_score"], batch["batch_cand"])
        hist_size = torch.tensor(
            [torch.where(mask_hist[i])[0].shape[0] for i in range(mask_hist.shape[0])],
            device=self.device,
        )
        user_mean_senti_score = torch.div(senti_hist.sum(dim=1), hist_size)
        senti_div_loss = F.relu(
            user_mean_senti_score.unsqueeze(dim=-1) * senti_cand * scores
        ).mean()
        loss += self.hparams.senti_div_loss_coef * senti_div_loss
