...
class TANRPLMModule(LightningModule):
    def __init__(
        self,
        plm_model: str,
        ...
        self.topic_predictor = Linear(self.hparams.text_embedding_dim, self.hparams.num_topics)
    ...
    def model_step(self, batch: MINDRecBatch) -> Tuple[
        ...
        topics = torch.cat((batch["x_cand"]["category"], batch["x_hist"]["category"]))
        topic_prob = F.one_hot(topics, num_classes=self.hparams.num_topics).type_as(topic_scores)
