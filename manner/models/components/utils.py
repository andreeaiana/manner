...
def pairwise_cosine_similarity(x: torch.Tensor, y: torch.Tensor, zero_diagonal: bool = False) -> torch.Tensor:
    """
    ...
    Args:
        x (torch.Tensor): (batch_size, M, d)
        y (torch.Tensor): (batch_size, N, d)
        zero_diagonal (bool): Determines if the diagonal of the distance matrix should be set to zero.
    """
